...
export function buildJevRequest(position: Position, model: string, candidates: Placement[]) {
  ...
  return {
    model,
    state: {
      game: "Tetris on a 10x20 board. Choose a reachable placement. Full rows clear. Survive and clear as many lines as possible. Each new piece spawns at the top center in its initial orientation. The game ends if that piece overlaps fixed blocks at spawn; hold cannot rescue a blocked spawn. No wall kicks. Hold is allowed once per piece before locking. Holding swaps the active piece with the held piece, or takes the next piece if hold is empty. The replacement starts at the top in its initial orientation.",
